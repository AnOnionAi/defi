...
describe('pools page functionality', () => {
	...
	it('opening the modal and executing metamask', async () => {
		...
		await page.screenshot({
			path: `screenshots/pools/pools-metamask-${browserName}-close.png`,
			fullPage: true
		});
		...
		await overviewPage.screenshot({
			path: `screenshots/pools/pools-metamask-${browserName}-open.png`,
			fullPage: true
		});
		...
		await page.waitForTimeout(2000);
		await page.screenshot({
			path: `screenshots/pools/pools-metamask-${browserName}-closeApprove.png`,
			fullPage: true
		});
		...
		await overviewPageCompleted.screenshot({
			path: `screenshots/pools/pools-metamask-${browserName}-openWithValues.png`,
			fullPage: true
		});
	});
});
