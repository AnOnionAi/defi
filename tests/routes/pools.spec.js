...
test.describe('pools page functionality', () => {
	test('should open the pools page', async ({ page, browserName }) => {
		...
		await page.screenshot({
			path: `tests/screenshots/pools/pools-${browserName}.png`,
			fullPage: true
		});
	});

	test('pressing the unlock button and opening metamask modal - pools', async ({
		page,
		browserName
	}) => {
		...
		const locator = page.locator(
			'.flex > .self-start:nth-child(1) > .py-4 > .flex > .bg-green-500'
		);
		await locator.click();
