...
describe('vaults page functionality', () => {
	...
	it('Using the drop down, Putting in values and Opening metamask with those values', async () => {
		...
		await page.waitForTimeout(2000);
		await page.screenshot({ path: `screenshots/vaults/vaults-metamask-${browserName}-close.png`, fullPage: true });
		...
		await overviewPageCompleted.screenshot({ path: `screenshots/vaults/vaults-metamask-${browserName}-open-withValues.png`, fullPage: true });
