...
contract StrategyMush is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(address _vaultChefAddress, address _wantAddress) public {
        ...
        returns (uint256)
    {
        ...
        uint256 pending = user.shares.mul(accUsdPerShare).div(1e18).sub(
            user.rewardDebt
        );
    ...
    function depositReward(uint256 _depositAmt) external returns (bool) {
        ...
        accUsdPerShare = accUsdPerShare.add(
            _depositAmt.mul(1e18).div(sharesTotal)
        );
