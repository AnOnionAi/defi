...
contract StrategyAave is Ownable, ReentrancyGuard, Pausable {
    ...
    address public constant aaveDataAddress =
        0x7551b5D2763519d4e37e8B81929D336De671d46d;
    address public constant aaveDepositAddress =
        0x8dFf5E27EA6b7AC08EbFdf9eB090F32ee9a30fcf;
    address public constant aaveClaimAddress =
    ...
    constructor(
        ...
        ) = IProtocolDataProvider(aaveDataAddress).getReserveConfigurationData(
                wantAddress
            );
    ...
    function _supply(uint256 _amount) internal {
        IAaveStake(aaveDepositAddress).deposit(
            wantAddress,
            _amount,
            address(this),
            referralCode
    ...
    function _removeSupply(uint256 _amount) internal {
        IAaveStake(aaveDepositAddress).withdraw(
            wantAddress,
            _amount,
            address(this)
        );
    ...
    function _borrow(uint256 _amount) internal {
        IAaveStake(aaveDepositAddress).borrow(
            wantAddress,
            _amount,
            2,
            referralCode,
    ...
    function _repayBorrow(uint256 _amount) internal {
        IAaveStake(aaveDepositAddress).repay(
            wantAddress,
            _amount,
            2,
            address(this)
    ...
    function earn() external nonReentrant whenNotPaused onlyGov {
        ...
        // Harvest farm tokens
        IAaveStake(aaveClaimAddress).claimRewards(
            vTokenArray,
            uint256(-1),
            address(this)
        );
        ...
        uint256 earnedAmt = IERC20(earnedAddress).balanceOf(address(this)).sub(
            preEarn
        );
    ...
    function wantLockedTotal() public view returns (uint256) {
        return wantLockedInHere().add(vTokenTotal()).sub(debtTotal());
    }
