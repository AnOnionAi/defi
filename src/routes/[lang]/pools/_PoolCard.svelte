<script lang="ts">
    ...
    import {approveToken,getTokenAllowance,isNotZero,getTokenBalance} from "../../../utils/erc20"
    ...
    $: wantDepositAmount: {
        console.log(wantDepositAmount)
    }

    const unsubscribe = accounts.subscribe(async(arrayAccs) => {
        if(arrayAccs){
            userAcc = arrayAccs[0];
            tokenAllowance = await getTokenAllowance(tokenAddr,"0x96306fa6C17A5edfA80C679051E3CA980A2e9CC9",userAcc);
    ...
    onMount(async()=>{
            poolTokenLiquidity = await getTokenBalance(tokenAddr,"0x96306fa6C17A5edfA80C679051E3CA980A2e9CC9");
    })
...
<div class=" flex flex-col justify-between shadow-l dark:bg-dark-600 rounded-xl space-y-2 border dark:border-0 ">
    ...
    <div class="dark:bg-dark-300 bg-gray-100 p-2 space-y-3">
        ...
        <div class="px-5">
            ...
            <div class="flex pt-3 justify-between">
                ...
                <p 
                on:click={async() => await deposit(pid,ethers.constants.Zero)}
                class="text-green-400 font-semibold text-lg tracking-wider p-1 cursor-pointer">Harvest</p>
            </div>
            </div>
        ...
        <div class="flex justify-between px-5">
            <p class="tabular-nums text-xl p-1 font-medium">0</p>
            <p
            on:click={showModal}
            class="text-green-400 font-semibold text-lg tracking-wider p-1 cursor-pointer  {!canStake && 'invisible'}">Deposit</p>        
        </div>
    {:else if  $accounts}
    <a  
        on:click={()=>approveToken(tokenAddr,"0x96306fa6C17A5edfA80C679051E3CA980A2e9CC9")}
        href="#"
       ...
       Unlock
        ...
        {/if}
           ...
           <div class="flex justify-center p-4" on:click={showPoolInfo}>
            ...
            <div class="flex cursor-pointer">
                <p>Hide</p>
                <Fa icon={faChevronUp} size="xs" scale={0.9} translateX={0.5} translateY={0.60}/>
            </div>
             {/if}
           </div>
