...
<a href={swapperURL} target="_blank" class="flex items-center group">
	{#if $tokenPrice}
		...
		/>
		<p class="pl-2 font-medium text-gray-700 dark:text-gray-200 ">${$tokenPrice.toPrecision(6)}</p>
	{:else}
		<div class="h-8 w-24 rounded-lg bg-gray-200 dark:bg-dark-400 animate-pulses" />
	{/if}
</a>
