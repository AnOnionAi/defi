...
export const getPriceOfMushPair = async(address: string) => {
    ...
    const token0 = {address: tokenAddresses[0] , reserve: reserves[0], ... await getTokenInformation(tokenAddresses[0]) }
    const token1 = {address: tokenAddresses[1] , reserve: reserves[1],  ... await getTokenInformation(tokenAddresses[1])}
