...
export const getERC20Contract = (address: string, signer: ethers.providers.Provider | ethers.Signer):ethers.Contract =>  new ethers.Contract(address, ERC20ABI, signer);

export const getTokenAllowance = async(tokenAddress:string, spenderAddr:string,userAddress:string):Promise<BigNumber> => {
...
export const getTokenBalance = async(tokenAddress:string, userAddress:string):Promise<BigNumber> => {
	const tokenContract = getERC20Contract(tokenAddress,Provider.getProviderSingleton());
...
export const approveToken = async(tokenAddress:string,spenderAddress:string,approvedAmount=ethers.constants.MaxUint256) => {
